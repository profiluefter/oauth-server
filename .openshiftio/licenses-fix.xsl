...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="license">
        <xsl:choose>
            ...
            <xsl:when
                    test="contains(translate(url/text(), $uppercase, $lowercase), 'www.apache.org/licenses/license-2.0')">
                <xsl:call-template name="license">
                    <xsl:with-param name="name" select="$apache_v2_name"/>
                    <xsl:with-param name="url" select="$apache_v2_url"/>
                </xsl:call-template>
            ...
            <xsl:when
                    test="contains(translate(., $uppercase, $lowercase), 'www.gnu.org/licenses/old-licenses/lgpl-2.1')">
                <xsl:call-template name="license">
