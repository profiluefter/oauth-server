...
public class HttpApplicationTest {
	...
	public void before(TestContext context) {
		...
		vertx.deployVerticle(HttpApplication.class.getName(), new DeploymentOptions().setConfig(new JsonObject().put(
				"http.port", PORT)), context.asyncAssertSuccess());
		client = WebClient.create(vertx); logger = LoggerFactory.getLogger(this.getClass());
	}
	...
	public void testLogin(TestContext context) {
		...
		client.get(PORT, "localhost", "/api/login?username=fabian&password=todo").send(resp -> {
			context.assertTrue(resp.succeeded()); context.assertEquals(200, resp.result().statusCode());
			String message = resp.result().bodyAsJsonObject().getString("message"); context.assertEquals(message,
					"ok");
			...
			client.get(PORT, "localhost", "/api/me").putHeader("Cookie", cookie.split(";")[0]).send(event -> {
				context.assertTrue(event.succeeded()); context.assertEquals(200, event.result().statusCode());
				...
				logger.info("Successfully completed login test!"); async.complete();
			});
		});
	}
