...
@SuppressWarnings("ALL") //Autogenerated by Openshift
...
public class OpenShiftIT {
	...
	@Before
	public void setup() {
		RestAssured.baseURI = route.toString(); project = this.client.getNamespace();
	}
	...
	public void testThatWeAreReady() {
		await().atMost(5, TimeUnit.MINUTES).until(() -> {
			...
			return list.stream().filter(pod -> pod.getMetadata().getName().startsWith(applicationName)).filter(this::isRunning).collect(Collectors.toList()).size() >= 1;
		});
		// Check that the route is served.
		await().atMost(5, TimeUnit.MINUTES).catchUncaughtExceptions().until(() -> get().getStatusCode() < 500);
		await().atMost(5, TimeUnit.MINUTES).catchUncaughtExceptions().until(() -> get("/api/greeting").getStatusCode() < 500);
	}
